...
export function PODetailsPage({ poId, onBack }: PODetailsPageProps) {
  const { user } = useAuth();
  const [po, setPo] = useState<PORecordFull | null>(null);
  ...
  useEffect(() => {
    ...
    setLoadError(null);
    Promise.all([
      api.get<PORecordFull>(`/api/v1/po-records/${poId}`),
      ...
      setLoadError(err instanceof Error ? err.message : 'Could not load purchase order. Check your connection.');
      setIsLoading(false);
    });
  }, [poId]);
  ...
  const totalAmount =
    po && po.price_per_unit && po.quantity_bags && po.bag_weight_kg
      ? (
  ...
  if (loadError || !po) {
    return (
      <div className="space-y-4">
        <button onClick={onBack} className="flex items-center gap-2 p-2 hover:bg-accent rounded-lg transition-colors">
