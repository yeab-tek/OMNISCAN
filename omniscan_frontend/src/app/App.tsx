...
function AppShell() {
  ...
  const renderPage = () => {
    switch (currentPage) {
      ...
      case 'po-details':
        return <PODetailsPage poId={selectedPoId} onBack={() => setCurrentPage('purchase-orders')} />;
      default:
        return <DashboardPage />;
    }
  };
  ...
  return (
    <ThemeProvider>
      <NotificationsProvider>
        <OcrResultProvider>
          <Layout currentPage={currentPage} onNavigate={handleNavigate} userRole={user?.role} onLogout={logout}>
            {renderPage()}
          </Layout>
          <Toaster position="top-right" richColors />
        </OcrResultProvider>
