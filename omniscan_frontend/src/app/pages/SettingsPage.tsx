...
function Toggle({ checked, onChange }: ToggleProps) {
  return (
    ...
    >
      <span
        className={`inline-block w-5 h-5 bg-white rounded-full shadow transform transition-transform duration-200 mt-0.5 ${checked ? "translate-x-5" : "translate-x-0.5"}`}
      />
    </button>
  );
}
              ...
              <div className="bg-card border border-border rounded-xl p-6">
                ...
                <div className="flex items-center gap-5 mb-6 pb-6 border-b border-border">
                  <div className="relative">
                    <div className="w-20 h-20 rounded-full bg-primary text-primary-foreground flex items-center justify-center text-2xl overflow-hidden">
                      {avatarUrl ? (
                        <img
                          src={avatarUrl}
                          alt="Avatar"
                    ...
                    <button
                      ...
                      className="absolute -bottom-1 -right-1 w-7 h-7 bg-card border border-border rounded-full flex items-center justify-center hover:bg-accent transition-colors shadow-sm"
                    >
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  <div>
                    ...
                    >
                      ...
                      <option>UTC+3 (Addis Ababa, Ethiopia)</option>
              ...
              <div className="bg-card border border-border rounded-xl p-6">
                ...
                <div className="mb-6">
                  ...
                  <SettingRow
                    ...
                    onChange={(v) =>
                      setNotifPrefs({ ...notifPrefs, pushNotifications: v })
                    }
                  />
                </div>
                <div className="mb-6">
                  ...
                  <SettingRow
                    ...
                    description="Upcoming shipment and departure alerts"
                    checked={notifPrefs.shipmentReminders}
                    onChange={(v) =>
                      setNotifPrefs({ ...notifPrefs, shipmentReminders: v })
                    }
                  />
                  <SettingRow
                    title="Payment Alerts"
                    description="Overdue invoices and payment due dates"
                    checked={notifPrefs.paymentAlerts}
                    onChange={(v) =>
                      setNotifPrefs({ ...notifPrefs, paymentAlerts: v })
                    }
                  />
                  <SettingRow
                    title="Compliance Alerts"
                    description="EUDR documentation and compliance updates"
                    checked={notifPrefs.complianceAlerts}
                    onChange={(v) =>
                      setNotifPrefs({ ...notifPrefs, complianceAlerts: v })
                    }
                  />
                  <SettingRow
                    title="OCR Completion"
                    description="Notify when OCR processing finishes"
                    checked={notifPrefs.ocrCompletion}
                    onChange={(v) =>
                      setNotifPrefs({ ...notifPrefs, ocrCompletion: v })
                    }
                  />
                </div>
                <div>
                  ...
                  <SettingRow
                    ...
                    description="Weekly summary digest every Monday"
                    checked={notifPrefs.weeklyReports}
                    onChange={(v) =>
                      setNotifPrefs({ ...notifPrefs, weeklyReports: v })
                    }
                  />
                  <SettingRow
                    title="Monthly Reports"
                    description="Monthly performance overview"
                    checked={notifPrefs.monthlyReports}
                    onChange={(v) =>
                      setNotifPrefs({ ...notifPrefs, monthlyReports: v })
                    }
                  />
                  <SettingRow
                    title="New User Alerts"
                    description="When new users are added to the system"
                    checked={notifPrefs.newUserAlerts}
                    onChange={(v) =>
                      setNotifPrefs({ ...notifPrefs, newUserAlerts: v })
                    }
                  />
                  <SettingRow
                    title="System Maintenance"
                    description="Scheduled downtime and maintenance windows"
                    checked={notifPrefs.systemMaintenance}
                    onChange={(v) =>
                      setNotifPrefs({ ...notifPrefs, systemMaintenance: v })
                    }
                  />
                </div>
              </div>
            ...
            <>
                  ...
                  {themeOptions.map((opt) => {
                    ...
                    return (
                      <button
                        ...
                        className={`relative flex flex-col items-center gap-2 p-4 rounded-xl border-2 transition-all ${isSelected ? "border-primary bg-primary/5" : "border-border hover:border-primary/40 hover:bg-accent"}`}
                      >
                        {isSelected && (
                        ...
                        <div
                          className={`w-10 h-10 rounded-full flex items-center justify-center ${isSelected ? "bg-primary text-primary-foreground" : "bg-muted text-muted-foreground"}`}
                        >
              ...
              <div className="bg-card border border-border rounded-xl p-6">
                ...
                <SettingRow
                  ...
                  onChange={(v) => setSystem({ ...system, compactMode: v })}
                />
                <SettingRow
                  title="Animations"
                  description="Enable UI motion and transitions"
                  checked={system.animationsEnabled}
                  onChange={(v) =>
                    setSystem({ ...system, animationsEnabled: v })
                  }
                />
              </div>
            </>
            ...
            <>
              ...
              <div className="bg-card border border-border rounded-xl p-6">
                ...
                <SettingRow
                  ...
                  onChange={(v) => setSystem({ ...system, autoSave: v })}
                />
                <SettingRow
                  title="Beta Features"
                  description="Access new features before general release"
                  checked={system.betaFeatures}
                  onChange={(v) => setSystem({ ...system, betaFeatures: v })}
                />
                <SettingRow
                  title="Debug Mode"
                  description="Enable developer console logging"
              ...
              <div className="bg-muted/50 border border-border rounded-xl p-4">
                <p className="text-xs text-muted-foreground text-center">
                  OmniScan v2.0.0 · Built for LATA AGRI EXPORT · © 2025
                </p>
              </div>
            </>
          )}
