import { Upload, File, X, CheckCircle, Loader, Image, FileText, Lock } from 'lucide-react';
import { useRef, useState } from 'react';
import { API_BASE, getToken } from '../lib/api';
import { useAuth } from '../context/AuthContext';
import { can } from '../lib/permissions';
import { useOcrResult, extractedFieldsToList } from '../context/OcrResultContext';
import { toast } from 'sonner';

type DocType = 'po_record' | 'commercial_invoice' | 'bill_of_lading' | 'packing_list' | 'trade_certificate';
...
export function UploadPage({ onNavigate }: UploadPageProps) {
    ...
    return (
      <div className="flex flex-col items-center justify-center py-24 gap-4 text-center">
        ...
        <p className="text-muted-foreground max-w-sm">
          Your role does not have permission to upload documents. Contact your System Admin if you believe this is a mistake.
        </p>
      </div>
    );
  }
  ...
  const processFile = async (file: File) => {
    ...
    setFiles((prev) => [...prev, { id, name: file.name, size: sizeLabel, status: 'uploading', confidence: null, jobId: null }]);
    ...
    try {
      ...
      setFiles((prev) => prev.map((f) => (f.id === id ? { ...f, status: 'processing', jobId: data.job_id } : f)));
      pollJob(id, data.job_id, file.name);
    } catch {
      // Demo fallback when backend isn't reachable
      setTimeout(() => setFiles((prev) => prev.map((f) => (f.id === id ? { ...f, status: 'processing' } : f))), 600);
      setTimeout(() => {
        setFiles((prev) => prev.map((f) => (f.id === id ? { ...f, status: 'complete', confidence: 0.94 } : f)));
        setResult({
          docType,
          fileName: file.name,
          fileUrl: null,
          fields: extractedFieldsToList(docType, DEMO_EXTRACTED_FIELDS[docType] || {}, 0.94),
        });
        toast.info('Demo mode: OCR processing simulated (backend not reachable)', {
          action: { label: 'Review', onClick: () => onNavigate('ocr-results') },
        });
      }, 2200);
    }
  };
  ...
  const pollJob = (fileId: string, jobId: string, fileName: string) => {
    ...
    const interval = setInterval(async () => {
      try {
        ...
        if (!res.ok) return;
        const data: { status: string; confidence?: number; error?: string; extracted_fields?: Record<string, unknown>; file_url?: string; doc_type?: string } = await res.json();
        if (data.status === 'done') {
          clearInterval(interval);
          const confidence = data.confidence ?? 0;
          setFiles((prev) => prev.map((f) => (f.id === fileId ? { ...f, status: 'complete', confidence } : f)));
          setResult({
            docType: data.doc_type || docType,
            fileName,
            fileUrl: data.file_url || null,
            fields: extractedFieldsToList(data.doc_type || docType, data.extracted_fields || {}, confidence),
          });
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-muted-foreground">Upload purchase order documents for OCR processing</p>
      </div>

      {/* Document Type Selector */}
      <div className="bg-card border border-border rounded-lg p-6">
        <label className="block text-sm mb-2 text-foreground">Document Type</label>
        <select
          value={docType}
      ...
      {/* Upload Area */}
      <div
        className={`bg-card border-2 border-dashed rounded-lg p-12 text-center transition-colors cursor-pointer ${isDragging ? 'border-primary' : 'border-border hover:border-primary'}`}
        onDrop={handleDrop}
        onDragOver={handleDragOver}
        onDragLeave={() => setIsDragging(false)}
        onClick={() => fileInputRef.current?.click()}
      >
        <input ref={fileInputRef} type="file" multiple accept=".jpg,.jpeg,.png,.pdf" className="hidden" onChange={handleFileInput} />
        <div className="max-w-md mx-auto">
          ...
          <button type="button" className="px-6 py-2 bg-primary text-primary-foreground rounded-lg hover:opacity-90 transition-opacity">
            Select Files
          </button>
        </div>
      </div>
      ...
      {files.length > 0 && (
        <div className="bg-card border border-border rounded-lg p-6">
          ...
          <div className="space-y-3">
            {files.map((file) => (
              <div key={file.id} className="p-4 bg-background rounded-lg border border-border">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    {file.status === 'processing' && (
                      <div className="flex items-center gap-2 text-warning">
                        <Loader className="w-4 h-4 animate-spin" />
                        <span className="text-sm">Processing with AI OCR...</span>
                      </div>
                    )}
                    {file.status === 'complete' && (
                      <div className="flex items-center gap-2 text-success">
                        ...
                        <span className="text-sm">
                          Processing complete{file.confidence != null ? ` — Confidence: ${Math.round(file.confidence * 100)}%` : ''}
                        </span>
                      </div>
                    )}
                    {file.status === 'failed' && (
            ...
            >
              ...
              <div className="flex items-center gap-4">
                <div className="text-right">
                  ...
                  <p className="font-medium text-success">{upload.confidence}%</p>
                </div>
