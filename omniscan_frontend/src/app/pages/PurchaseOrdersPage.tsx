...
const FALLBACK: PORecord[] = [
  {
    ...
    quality_description: "Ethiopia Djimmah GR 5",
    quantity_bags: 160,
  ...
  {
    id: "2",
    po_number: "EFI25120040",
    buyer_name: "Global Coffee Co",
    seller_name: "LATA AGRI EXPORT",
    quality_description: "Arabica Grade 1",
    quantity_bags: 200,
    shipment_start: "2026-01-14",
  ...
  {
    id: "3",
    po_number: "EFI25120041",
    buyer_name: "Premium Foods",
    seller_name: "LATA AGRI EXPORT",
    quality_description: "Djimmah Grade 4",
    quantity_bags: 80,
    shipment_start: "2026-01-15",
...
export function PurchaseOrdersPage({ onNavigate }: PurchaseOrdersPageProps) {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const data = await api.get<POListResponse>(
        `/api/v1/po-records?${params}`,
      );
      setPos(data.items);
      setTotal(data.total);
      setPages(Math.max(1, data.pages || Math.ceil(data.total / limit)));
  ...
  const handleApprove = async (id: string) => {
    setActioningId(id);
    try {
      await api.patch(`/api/v1/po-records/${id}/approve`);
      toast.success("Purchase order approved");
      load();
    } catch (err) {
      toast.error(
        err instanceof ApiError ? err.message : "Could not approve this PO",
      );
    } finally {
      setActioningId(null);
    }
  ...
  const handleReject = async (id: string) => {
    setActioningId(id);
    try {
      await api.patch(`/api/v1/po-records/${id}/reject`);
      toast.info("Purchase order rejected");
      load();
    } catch (err) {
      toast.error(
        err instanceof ApiError ? err.message : "Could not reject this PO",
      );
    } finally {
      setActioningId(null);
    }
  ...
  const handleExport = async (format: "pdf" | "excel") => {
    try {
      const blob = await api.blob(`/api/v1/reports/po/export?format=${format}`);
      downloadBlob(
        blob,
        `omniscan_po_report.${format === "pdf" ? "pdf" : "xlsx"}`,
      );
      ...
      toast.error(
        err instanceof ApiError
          ? err.message
          : "Export failed — is the backend running?",
      );
    }
  };
      ...
      <div className="flex items-center justify-between flex-wrap gap-4">
        <div>
          ...
          <p className="text-muted-foreground">
            Manage and track all purchase order records
          </p>
        </div>
              ...
              {isLoading && pos.length === 0 ? (
                ...
                pos.map((po) => (
                  ...
                  >
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      {getEudrBadge(po.eudr_compliant)}
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap">
