...
import { can, roleLabel, roleBadgeVariant, PERMISSIONS, Role } from '../lib/permissions';
...
const FALLBACK: SystemUser[] = [
  { id: '1', full_name: 'System Admin', email: 'admin@lataagriexport.com', role: 'system_admin', phone_number: null, is_active: true, last_login: '2026-06-19T14:23:00Z' },
  { id: '2', full_name: 'Data Entry Operator', email: 'dataentry@lataagriexport.com', role: 'data_entry_operator', phone_number: null, is_active: true, last_login: '2026-06-19T13:45:00Z' },
  { id: '3', full_name: 'Trade Manager', email: 'trade@lataagriexport.com', role: 'trade_manager', phone_number: null, is_active: true, last_login: '2026-06-19T12:10:00Z' },
  { id: '4', full_name: 'Finance Officer', email: 'finance@lataagriexport.com', role: 'finance_officer', phone_number: null, is_active: true, last_login: '2026-06-19T11:30:00Z' },
];
...
const PERMISSION_ROWS: { label: string; key: keyof typeof PERMISSIONS['system_admin'] }[] = [
...
export function UsersPage() {
  ...
  const [newUser, setNewUser] = useState({ full_name: '', email: '', password: '', role: 'data_entry_operator' as Role, phone_number: '' });
  ...
  const load = async () => {
    ...
    try {
      ...
      const data = await api.get<SystemUser[]>(`/api/users${params.toString() ? `?${params}` : ''}`);
      setUsers(data);
  ...
  if (!canManage) {
    return (
      <div className="flex flex-col items-center justify-center py-24 gap-4 text-center">
        ...
        <p className="text-muted-foreground max-w-sm">Only System Administrators can manage users.</p>
      </div>
    );
  }
  ...
  const handleCreate = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setNewUser({ full_name: '', email: '', password: '', role: 'data_entry_operator', phone_number: '' });
      load();
    } catch (err) {
      toast.error(err instanceof ApiError ? err.message : 'Could not create user — backend unreachable');
    } finally {
      setIsCreating(false);
    }
  };
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between flex-wrap gap-4">
        <div>
          ...
          <p className="text-muted-foreground">Manage users and their access roles</p>
        </div>
        <button
          ...
          <table className="w-full">
            <thead className="bg-muted border-b border-border">
              <tr>
                ...
                <th className="px-6 py-4 text-left text-xs font-medium text-muted-foreground uppercase tracking-wider">Last Login</th>
      ...
      <div className="bg-card border border-border rounded-lg p-6">
        <h3 className="font-semibold text-foreground mb-1">Role Permissions Matrix</h3>
        <p className="text-sm text-muted-foreground mb-4">What each role can do across the system</p>
        <div className="overflow-x-auto">
          <table className="w-full min-w-[640px]">
            <thead>
              <tr className="border-b border-border">
                <th className="px-3 py-2 text-left text-xs text-muted-foreground">Permission</th>
                {ROLE_OPTIONS.map((r) => (
                  <th key={r.value} className="px-3 py-2 text-center text-xs text-muted-foreground">{r.label}</th>
                ))}
              </tr>
            </thead>
            <tbody>
              {PERMISSION_ROWS.map((row) => (
                <tr key={row.key} className="border-b border-border">
                  <td className="px-3 py-3 text-sm text-foreground">{row.label}</td>
                  {ROLE_OPTIONS.map((r) => (
                    <td key={r.value} className="px-3 py-3 text-center">
                      {PERMISSIONS[r.value][row.key] ? (
                        <span className="inline-flex w-5 h-5 rounded-full bg-success/10 text-success items-center justify-center text-xs">✓</span>
                      ) : (
      ...
      {showCreateModal && (
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4" onClick={() => setShowCreateModal(false)}>
          <div className="bg-card border border-border rounded-xl p-7 w-full max-w-md max-h-[90vh] overflow-y-auto" onClick={(e) => e.stopPropagation()}>
