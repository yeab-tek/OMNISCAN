...
import { ArrowLeft, Download, Edit, FileText, Package, Building2, MapPin, Calendar, DollarSign, CheckCircle, Clock, User, Loader2 } from 'lucide-react';
...
export function PODetailsPage({ poId, onBack }: PODetailsPageProps) {
  ...
  useEffect(() => {
    ...
    setIsLoading(true);
    Promise.all([
      api.get<PORecordFull>(`/api/po/${poId}`).catch(() => null),
      api.get<AuditEntry[]>(`/api/po/${poId}/activity`).catch(() => []),
  ...
  const handleExport = async () => {
    try {
      const blob = await api.blob(`/api/reports/po/export?format=pdf&buyer=${encodeURIComponent(po.buyer_name || '')}`);
  ...
  const handlePaymentUpdate = async (newStatus: string) => {
    ...
    try {
      const updated = await api.patch<PORecordFull>(`/api/po/${poId}/payment`, { payment_status: newStatus });
      setPo(updated);
      toast.success(`Payment status updated to ${newStatus}`);
    } catch (err) {
      toast.error(err instanceof ApiError ? err.message : 'Could not update payment status');
    } finally {
  ...
  const paymentBadge = (status: string) => {
    if (status === 'received' || status === 'paid') return <Badge variant="success">Received</Badge>;
  ...
  const totalAmount =
    po.price_per_unit && po.quantity_bags && po.bag_weight_kg
      ? (Number(po.price_per_unit) * Number(po.quantity_bags) * Number(po.bag_weight_kg)).toLocaleString(undefined, { maximumFractionDigits: 2 })
      : null;
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between flex-wrap gap-4">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-3xl font-semibold text-foreground mb-1">{po.po_number}</h1>
      ...
      <div className="bg-card border border-border rounded-lg p-6">
        <div className="flex items-center justify-between flex-wrap gap-4">
          <div className="flex items-center gap-6 flex-wrap">
            ...
            <div className="w-px h-10 bg-border hidden sm:block"></div>
            <div>
              <p className="text-sm text-muted-foreground mb-1">Record Status</p>
              {statusBadge(po.status)}
            </div>
          </div>
          <div className="text-right">
            <p className="text-sm text-muted-foreground">Created</p>
            <p className="font-medium text-foreground">{po.created_at ? new Date(po.created_at).toLocaleDateString() : '—'}</p>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          <div className="bg-card border border-border rounded-lg p-6">
            <h3 className="font-semibold text-foreground mb-4">Trading Partners</h3>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
              <div>
                ...
                <div className="space-y-2 text-sm">
                  <p className="font-medium text-foreground">{po.buyer_name || '—'}</p>
                  <div className="flex items-start gap-2 mt-3">
                    <MapPin className="w-4 h-4 text-muted-foreground mt-0.5 flex-shrink-0" />
                    <p className="text-muted-foreground">{po.buyer_country || '—'}</p>
                  </div>
                </div>
              </div>
              <div>
                ...
                <div className="space-y-2 text-sm">
                  <p className="font-medium text-foreground">{po.seller_name || 'LATA AGRI EXPORT'}</p>
                  <div className="flex items-start gap-2 mt-3">
                    <MapPin className="w-4 h-4 text-muted-foreground mt-0.5 flex-shrink-0" />
          ...
          <div className="bg-card border border-border rounded-lg p-6">
            <div className="flex items-center gap-2 mb-4">
              ...
              <h3 className="font-semibold text-foreground">Commodity Details</h3>
            </div>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
              <div className="space-y-3">
                <div>
                  <p className="text-sm text-muted-foreground mb-1">Quality / Grade</p>
                  <p className="font-medium text-foreground">{po.quality_description || '—'}</p>
                </div>
                <div>
                  <p className="text-sm text-muted-foreground mb-1">Crop Year</p>
                  <p className="font-medium text-foreground">{po.crop_year || '—'}</p>
                </div>
                <div>
                  <p className="text-sm text-muted-foreground mb-1">Quantity</p>
                  <p className="font-medium text-foreground">
                    {po.quantity_bags ?? '—'} bags{po.bag_weight_kg ? ` (${po.bag_weight_kg}kg each)` : ''}
                  </p>
                </div>
              </div>
              <div className="space-y-3">
                <div>
                  ...
                  <p className="font-medium text-foreground">
                    {po.price_per_unit ? `${po.price_currency || 'USC'} ${po.price_per_unit}` : '—'}
                  </p>
                </div>
                <div>
                  <p className="text-sm text-muted-foreground mb-1">Estimated Total</p>
                  <p className="text-2xl font-semibold text-primary">
                    {totalAmount ? `${po.price_currency || 'USC'} ${totalAmount}` : '—'}
                  </p>
                </div>
              </div>
            </div>
          ...
          <div className="bg-card border border-border rounded-lg p-6">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div className="p-3 bg-background rounded-lg">
                <p className="text-sm text-muted-foreground mb-1">Shipment Start</p>
                <p className="font-medium text-foreground">{po.shipment_start || '—'}</p>
              </div>
              <div className="p-3 bg-background rounded-lg">
                <p className="text-sm text-muted-foreground mb-1">Shipment End</p>
                <p className="font-medium text-foreground">{po.shipment_end || '—'}</p>
              </div>
              <div className="p-3 bg-background rounded-lg">
                <p className="text-sm text-muted-foreground mb-1">Origin Port</p>
                <p className="font-medium text-foreground">{po.origin_port || '—'}</p>
              </div>
              <div className="p-3 bg-background rounded-lg">
                <p className="text-sm text-muted-foreground mb-1">Incoterms</p>
                <p className="font-medium text-foreground">{po.incoterms || '—'}</p>
              </div>
              <div className="p-3 bg-background rounded-lg sm:col-span-2">
                <p className="text-sm text-muted-foreground mb-1">Destination Port</p>
                <p className="font-medium text-foreground">{po.destination_port || '—'}</p>
              </div>
            </div>
          </div>
          ...
          {/* Linked Documents */}
          <div className="bg-card border border-border rounded-lg p-6">
            <h3 className="font-semibold text-foreground mb-4">Linked Documents</h3>
            <div className="flex items-center gap-4 p-4 bg-background rounded-lg border border-border">
              <div className="w-10 h-10 bg-primary/10 rounded-lg flex items-center justify-center">
                <FileText className="w-5 h-5 text-primary" />
              </div>
              <div>
                <p className="font-medium text-foreground">Original PO Document</p>
                <p className="text-sm text-muted-foreground">
                  Uploaded {po.created_at ? new Date(po.created_at).toLocaleDateString() : ''}
                </p>
              </div>
            </div>
          </div>
            ...
            <div className="space-y-3">
              ...
              {can(user?.role, 'canUpdatePayment') && (
                <div className="p-3 bg-background rounded-lg border border-border">
                  <p className="text-sm text-muted-foreground mb-2">Update Payment Status</p>
            ...
            <div className="space-y-3">
              {activity.length === 0 ? (
                ...
                activity.slice(0, 8).map((a) => (
                  <div key={a.id} className="p-3 bg-background rounded-lg border border-border">
                    <div className="flex items-start gap-2 mb-1">
                      <User className="w-4 h-4 text-muted-foreground mt-0.5 flex-shrink-0" />
                      <div className="flex-1 min-w-0">
                        <p className="text-sm font-medium text-foreground truncate">{a.user_email || 'System'}</p>
                        <p className="text-sm text-muted-foreground">{(a.user_role || '').replace(/_/g, ' ')}</p>
                      </div>
                    </div>
                    <p className="text-sm text-foreground mt-2 capitalize">{a.action}</p>
